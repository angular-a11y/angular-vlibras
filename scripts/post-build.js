...
const outputDir = 'dist/angular-vlibras';
...
function minifyPackageJson() {
  try {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      console.log(`package.json not found at ${packageJsonPath}. Running 'npm run build:lib'...`);
      try {
        execSync('npm run build:lib', { stdio: 'inherit' });
      } catch (error) {
        console.error('Error running npm run build:lib:', error.message);
      }
      return;
    }
